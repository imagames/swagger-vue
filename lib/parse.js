...
let compareMethods = function (methodA, methodB) {
  const pathComparison = methodA.path.localeCompare(methodB.path)
  if (pathComparison === 0) {
    return methodA.method.localeCompare(methodB.method)
  }
...
let getViewForSwagger2 = function (opts) {
  ...
  let data = {
    ...
    methods: [],
    definitions: [],
    forcePathAsMethodName: opts.forcePathAsMethodName ? opts.forcePathAsMethodName : false
  }
  ...
  _.forEach(swagger.paths, function (api, path) {
    ...
    _.forEach(api, function (op, m) {
      ...
      let methodName = data.forcePathAsMethodName ? getPathToMethodName(opts, m, path) : (op.operationId ? normalizeName(op.operationId) : getPathToMethodName(opts, m, path))
      if (opts.excludeNamesRegex && methodName.match(opts.excludeNamesRegex)) {
        return
      }
      let method = {
        path: path,
  ...
  // sort methods and definitions to make the output more deterministic
  data.definitions.sort((a, b) => a.name.localeCompare(b.name))
  data.methods.sort(compareMethods)
